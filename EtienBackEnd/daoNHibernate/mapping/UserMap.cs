...
namespace daoNHibernate.mapping
{
    public class UserMap : ClassMap<UserEntity>
    {
        public UserMap()
        {
            ...
            Map(x => x.LastName).Column("LastName").Not.Nullable().CustomSqlType("longtext");
            Map(x => x.UserName).Column("UserName").Not.Nullable().CustomSqlType("longtext");
            Map(x => x.Password).Column("Password").Not.Nullable().CustomSqlType("longtext");
            Map(x => x.Email).Column("Email").Not.Nullable().CustomSqlType("longtext");
            Map(x => x.UserDeviceId).Column("UserDeviceId").Nullable().CustomSqlType("longtext");
            ...
            Map(x => x.AppToken).Column("Token").CustomSqlType("longtext");
